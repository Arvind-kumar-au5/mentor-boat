...
const onSubmit = (e) =>{
        ...
        const updateApplication = async () => {
          try {
            let result = await axios.post(`/api/applications/${mentorName}`, Application)
            console.log(result)
            if (result) {
        ...
        <div>
            ...
            <h4 style ={{textAlign:'center'}} className="text-primary mt-2">Mentorship Registeration </h4>
                ...
                <div className="col-lg-12 col-md-12 col-xs-12 mt-3">
                    {isAuthenticated ? 
                    <Fragment>
                        <TextField id="standard-basic" label="Name" style ={{width:'270px'}} name="Name" value= {Application.name} disabled/>
                    </Fragment>:
                    <Fragment>
                ...
                <div className="col-lg-12 col-md-12 col-xs-12 mt-3">
                    {isAuthenticated ? 
                    <Fragment>
                        <TextField id="standard-basic" label="Email" style ={{width:'270px'}} name="Email" type="email" value ={Application.email} disabled/>
                    </Fragment>:
                    <Fragment>
                ...
                <div className="col-lg-12 col-md-12 col-xs-12 mt-5">
                    ...
                    <textarea className="textarea" name="description" cols="40" rows="5"  
                        name = "bio" placeholder="bio" onChange = {(e)=>setApplication({bio:e.target.value,name,email,expectation,mentorName})}
                    >
                    </textarea>
                </div>
                <div className="col-lg-12 col-md-12 col-xs-12 mt-5">
                    <FormHelperText id="my-helper-text">Tell us (and your students) a little bit about yourself. This will be public. Please write any information your mentees might need in here. Also disclose if you want to mentor a specific minority. Keep it short, but not too short.</FormHelperText>
                    <textarea className="textarea" name="description" cols="40" rows="5"  
                        name = "bio" placeholder="expectation:" onChange = {(e)=>setApplication({expectation:e.target.value,name,email,bio,mentorName})}
                    >
                    </textarea>
                </div>
